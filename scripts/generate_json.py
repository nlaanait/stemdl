...
def generate_fcdensenet_json(random=False, growth_rate=64, kernel=[5,5], n_pool= 3, n_layers=[2,2,3,4,3,2,2], model= 'FCDenseNet_custom', output_channels=1, dropout_prob=0.2, name='', save=True):
    DB_conv_kernel = [3, 3]
    if model == 'FC-DenseNet56':
        ...
        if random:
            ...
            # 25 TFLOPS: kernel=5, growth=128, pool=3, n_layers= [2,2,3,4,3,2,2]
    ...
    pool = OrderedDict({'type': 'pooling', 'stride': [2, 2], 'kernel': [2, 2], 'pool_type': 'max','padding':'VALID'})
    ...
    for i in range(n_pool):
      ...
      #      conv_layers.append(('extra_conv_%s'%j, std_conv))
      #      conv_layers.append(('extra_conv_extra_%s'%j, std_conv))
        ...
        TD = OrderedDict({'type': "transition_down", 'conv':
                                ...
                                'features': n_filters,
                                'activation': 'relu', 'padding': 'VALID', 'batch_norm': False, 'dropout':dropout_prob},
                                'pool':pool})
    ...
    for i in range(n_pool):
        ...
        #n_filters_keep = growth_rate * n_layers_per_block[n_pool]
        n_filters_keep -= n_filters % 8
        TU = OrderedDict({'type': "transition_up", 'deconv':
                                {'type': 'deconvolutional', 'stride': [2, 2], 'kernel': [1, 1],
                                'features': n_filters_keep,'padding': 'VALID', 'upsample':pool['kernel'][0]}
                                })
...
if __name__ == '__main__':
    ...
    if len(args) > 1:
        ...
        generate_fcdensenet_json(growth_rate=64, kernel=[3,3])
        #generate_fcdensenet_json(model='FC-DenseNet103')   
