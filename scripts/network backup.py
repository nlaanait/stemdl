        ...
        for layer_name, layer_params in list(self.network.items())[1:]:
            with tf.variable_scope(layer_name, reuse=self.reuse) as scope:
                ...
                if layer_params['type'] == 'residual':
                    # since the rest of the operations are just convs:
                    # layer_params['type'] = 'convolutional'  # may not even be necessary
                    ...
                    # hidden layer 1
                    with tf.variable_scope("_layer1"):
                        hidden = self._do_conv(out, layer_params)
                    hidden = self._activate(input=hidden, name=scope.name+'_layer1', params=layer_params)
                    # hidden layer 2
                    with tf.variable_scope("_layer2"):
                    ...
                    # Now activate
                    out = self._activate(input=out, name=scope.name+'_layer2', params=layer_params)
