...
class NewDatasetTFRecords(object):
    """
    Handles training and evaluation data operations.  \n
    Data is read from a TFRecords filename queue.
    """
    ...
    def train_images_labels_batch(self, random_glimpses=True):
        """
        Returns: batch of images and labels to train on.
        """
        ...
        # Generate batch
        #TODO: Need to change num_threads so that it's determined from horovod total_rank
        dataset = tf.contrib.data.Dataset(self.filename).repeat()
        dataset = dataset.map(self._parser(), num_threads=10, output_buffer_size=2 * self.flags.batch_size)
        ...
        dataset = dataset.shuffle(buffer_size=5000 + 3 * self.flags.batch_size)
