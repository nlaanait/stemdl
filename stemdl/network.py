...
class ConvNet(object):
    ...
    def _batch_norm(self, input=None):
        ...
        if self.operation == 'train':
            ...
            output, mean, variance = tf.nn.fused_batch_norm(input, beta, gamma, 1.e-3, data_format='NCHW')
        if self.operation == 'eval':
            ...
            variance = self._cpu_variable_init('moving_variance', shape=shape, \
                                               initializer=tf.ones_initializer(), trainable=False)
            output = tf.nn.fused_batch_norm(input, beta, gamma, 1.e-3, mean=mean, variance=variance,data_format='NCHW',
                                            is_training=False)
        # Keep tabs on the number of weights
